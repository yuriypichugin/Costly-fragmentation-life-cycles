setwd('/Volumes/OSX/Users/pichugin/Projects/Fragmentation with costs/Local simulations/Classes abundance and groups size/Detrimental random risk cost 2020 LV/')
...
Data=read.csv("Combined_Risk_Scan_Detrimental.txt")
...
if(Write2File==1){svg(file = "Dec_ClassesVsRisk_v9.svg", height=pdfSizeY, width=pdfSizeX)}
if(Write2File==2){cairo_pdf(file="Dec_ClassesVsRisk_v9.pdf", height=pdfSizeY, width=pdfSizeX)}
# svg(file="Dec_ClassesVsRisk_v5.svg", height=pdfSizeY, width=pdfSizeX)
...
axis(1, at=SQX*10, labels=0.1*SQX)
...
# DX <- 0.4
# 
# if(Write2File==1){svg(file = "StatisticsSizes_Risk_v4.svg", height=pdfSizeY, width=pdfSizeX)}
# if(Write2File==2){cairo_pdf(file="StatisticsSizes_Risk_v4.pdf", height=pdfSizeY, width=pdfSizeX)}
# # svg(file="StatisticsSizes_Risk_v2.svg", height=pdfSizeY, width=pdfSizeX)
# 
...
# # mtext("Fragmentation risk", side=1, line=2, cex=1.6)
# if(Write2File>0){dev.off()}
# 
# # dev.off()
