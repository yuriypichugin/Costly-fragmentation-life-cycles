setwd('/Volumes/OSX/Users/pichugin/Projects/Fragmentation with costs/Local simulations/Classes abundance and groups size/Unimodal random loss cost 2020/')
...
Data=read.csv("Combined_Loss_Scan_Unimodal.txt")
...
if(Write2File==1){svg(file = "ClassesVsLoss_v9.svg", height=pdfSizeY, width=pdfSizeX)}
if(Write2File==2){cairo_pdf(file="ClassesVsLoss_v9.pdf", height=pdfSizeY, width=pdfSizeX)}
# # svg(file="ClassesVsLoss_v5.svg", height=pdfSizeY, width=pdfSizeX)
# 
# par(pty="s", mgp=c(2.5,0.6,0.0), xpd=T, mar=c(Bottom,Left, Top, Right), cex.axis=1.1)
# CLR_list=c(CLR_bin, CLR_equal, CLR_seed, CLR_other)
# 
# MyBP2<-barplot(ClassPartitions[,], col=CLR_list, axes=F, ann=F, xlab="Fragmentation loss", ylab="Fraction of fitness landscapes", cex.lab=1.6, xaxs="i", yaxs="i", border=NA, space=0)
# SQ=seq(0,1, length.out = 6)
# axis(2, at=SQ, labels=SQ)
# 
# SQX=seq(0, 18, length.out=5)
# PosX=SQX/length(SQX)*dim(ClassPartitions)[2]
# # axis(1, at=SQX, labels=SQX)
# axis(1, at=c(0.5,6.5,12.5,18.5), labels=c(0,6,12,18))
par(pty="s", mgp=c(2.4,0.6,0.0), xpd=T, mar=c(Bottom,Left, Top, Right), cex.axis=1.4, bty="o", las = 1)
CLR_list=c(CLR_bin, CLR_equal, CLR_seed, CLR_other)

MyBP2<-barplot(ClassPartitions[,], col=CLR_list, axes=F, ann=F, xlab="Fragmentation loss", ylab="Fraction of environments", cex.lab=1.6, xaxs="i", yaxs="i", border=NA, space=0)
# Xarray <- seq(0, 18, length.out = dim(Data)[2])
# plot( c(0,18), c(0,1), type = "l", col="white", lwd = 1, ylim=c(0,1), xaxs="i", yaxs="i", ann=F)
...
# title(xlab="Fragmentation loss", ylab="Fraction of environments", cex.lab=1.6)
...
#   }
#   MeanFragSize <- c(MeanFragSize, mean(FragSize) + Cost - 1)
...
# 
# Xseq <- seq(0,18, length.out = length(MeanFragSize))
# SubsetSeq <- Xseq
# DX <- 0.4
# 
# if(Write2File==1){svg(file = "StatisticsSizes_Loss_v5.svg", height=pdfSizeY, width=pdfSizeX)}
# if(Write2File==2){cairo_pdf(file="StatisticsSizes_Loss_v5.pdf", height=pdfSizeY, width=pdfSizeX)}
# # svg(file="StatisticsSizes_Loss_v3.svg", height=pdfSizeY, width=pdfSizeX)
# 
# par(pty="s", mgp=c(2.5,0.6,0.0), xpd=T, mar=c(Bottom,Left, Top, Right+2), cex.axis=1.1, tcl = -0.5)
# plot(c(0,18), c(0,20), type="l", col="white", ann=F, xaxt="n", yaxt="n")
# 
...
# # axis(1, at=c(0, 6, 12, 18), labels=c(0, 6, 12, 18))
# par(tcl = 0.5)
# axis(1, at=c(0, 6, 12, 18), labels=F)
...
# # mtext("Fragmentation loss", side=1, line=2, cex=1.6)
# if(Write2File>0){dev.off()}
# 
# # dev.off()
